...
public class Exercise3Test extends ClassicOnlineStore {
    ...
    public void howManyItemsWanted() {
         ...
         */
        long sum = customerList.stream().map(mapper->mapper.getWantToBuy()).flatMap(mapper->mapper.stream()).count();
    ...
    public void richestCustomer() {
        ...
        Optional<Integer> richestCustomer = customerList.stream().map(sdfo->sdfo.getBudget()).max(comparator);
    ...
    public void youngestCustomer() {
         ...
         * Don't use {@link Stream#sorted}
         */
        Comparator<Customer> comparator = (cus1,cust2)->cus1.getAge().compareTo(cust2.getAge());
